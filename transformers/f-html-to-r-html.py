...
if use_paren_quote:
    QUOTE_S = "“"
    QUOTE_E = "”"
    PG_BRACKET_S = "[\(]" + QUOTE_S
    PG_BRACKET_E = QUOTE_E + "[\)]"
    PG_MIDDLE = "[^\)]+"
else:
    QUOTE_S = ""
    QUOTE_E = ""
...
PG_BRACKET_INSTRUCTION = (
    "[\[\(]" + "(?P<in>emphasize[^\]\)]*|pause|read slowly[^\]\)]*)" + "[\]\)]"
)
...
if use_tags:
    ...
    PG_BRACKET_INSTRUCTION = (
        PG_TAG_S + EXTRA_TAGS + PG_BRACKET_INSTRUCTION + EXTRA_TAGS + PG_TAG_E + "\s"
    )

else:
    PG_SB = "(?P<ss>" + SPACES + ")" + "(?P<sb>" + PG_BRACKET_S + ")"
    PG_M = "(?P<m>" + PG_MIDDLE + ")"
    PG_EB = "(?P<eb>" + PG_BRACKET_E + ")" + "(?P<es>" + SPACEM + ")"
...
def bracket_instruction(b):
    return re.sub(
        PG_BRACKET_INSTRUCTION,
        '<small class="bracket-instruction">(\g<in>)</small> ',
        b,
    )
...
def html_span_to_ruby(contents):
    ...
    for match in instances:
        ...
        formattedText += prev1 + prev2a + ruby_str
